...
class board:
	...
	def __repr__(self):
		...
		for i in range(0,8):
			for j in range(0,8):
				# the purpose of these if branches is to prettify the graphical representation of the board
				if(j == 0):
						boardStr += "|_"
				if str(self.board[i][j])== "0":
						...
						tileToBeAdded = "Q"
				if(j < 7):
						boardStr += tileToBeAdded + "_|_"
				else:
						 boardStr += tileToBeAdded + "_|"
			boardStr += "\n"
		return (boardStr)
...
class queens:
	def __init__(self, numruns, verbocity, passedboard=None):
		...
		for i in range(0,numruns):
			...
			self.hill_solution()
	def getverbocity(self): return self.verbocity
...
if __name__ == "__main__":
	...
	mboard = queens(verbocity=False, numruns=100)
	mboard.printstats()

# while mboard.cost  != 0:
# 	mboard = queens(verbocity=False, numruns=options.numrun)
